...
#include "../include/rescue_vision_24/hazmat.hpp"
...
{
  vision_rescue_24::HAZMAT start(argc, argv);
  start.run();
}
...
HAZMAT::HAZMAT(int argc, char** argv) : init_argc(argc), init_argv(argv), isRecv(false)
{
...
bool HAZMAT::init()
{
  ...
  img_result = img.advertise("/hazmat_image", 100);
  n.getParam("/usb_cam/image_raw", param);
  ROS_INFO("Starting Rescue Vision With Camera : %s", param.c_str());
  img_sub = img.subscribe("/usb_cam/image_raw", 100, &HAZMAT::imageCallBack, this);  /// camera/color/image_raw
